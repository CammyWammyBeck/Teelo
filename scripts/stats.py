...
def get_quarters_since_played(player_name, match_id, conn):
    ...
    query = f"SELECT match_id FROM tennis_matches WHERE (A_name = '{player_name_query}' \
        OR B_name = '{player_name_query}')"
...
def get_h2h(player_A_name, player_B_name, match_id, matches):
    ...
    for match in matches:
        if (
            ...
            continue
        if match["A_name"] == player_A_name and match["B_name"] == player_B_name:
            player_A_h2h += 1
        elif match["A_name"] == player_B_name and match["B_name"] == player_A_name:
            player_B_h2h += 1
        ...
        if serve_rating_margin is not None:
            ...
            filtered_matches = [
                ...
                if (
                    ...
                    <= serve_rating + margin
                    and player_name == match["A_name"]
                )
                or (
                    serve_rating - margin
                    <= match["B_avg_serve_rating"]
                    <= serve_rating + margin
                    and player_name == match["B_name"]
                )
            ]

        wins = len(
            [match for match in filtered_matches if match["A_name"] == player_name]
        )
        losses = len(
            [match for match in filtered_matches if match["A_name"] != player_name]
        )
